...
Take a look at the `request handlers` in the [file=0]`src/main/java/io/openliberty/guides/microprofile/InventoryResource.java` file.
...
Take a look at your [file=0]`InventoryManager` and [file=1]`InventoryUtil` files.
...
The [hotspot=buildHostJson file=1]`buildHostJson` helper method builds in the [file=1]`InventoryUtil` class. It
...
`localhost`. If the inventory does not contain an entry for the host name that is specified in the URL, which is
...
The [hotspot=assertResponse]`assertResponse` helper method ensures that the response that you receive is
valid, with a `200` response code.
