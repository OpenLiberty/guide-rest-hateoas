...
:page-seo-description: A getting started tutorial on how to define and traverse through hypermedia links in a RESTful Java microservice's JSON response to navigate to the appropriate resource using Hypermedia as the Engine of Application State (HATEOAS).
...
Navigate to the `start` directory. Start Open Liberty in development mode:
...
method and integrate it into the [hotspot=PropertiesForHost file=0]`getPropertiesForHost` method.
...
The [hotspot=UriInfo file=0]`UriInfo` object is what is used to build your HATEOAS links.
...
Your new [hotspot=InventoryResource file=0]`InventoryResource` class is now replaced. Next, let's implement the [hotspot=getSystems file=1]`getSystems` method and build
...
The [hotspot=buildHostJson file=1]`buildHostJson` helper method builds in the `InventoryUtil` class. It
creates a JSON object that contains the [hotspot=hostname file=1]`hostname` of your system and the [hotspot=links file=1]`_links` array, 
which is generated separately in the [hotspot=buildLinksForHost file=1]`buildLinksForHost` helper method.

This helper accepts a hostname and a target URL as arguments. The helper builds a link that points to the
inventory entry with a [hotspot=self file=1]`self` relationship and also builds a link that points to the `system` service with
a [hotspot=properties file=1]`properties` relationship:
...
You can use the [hotspot=Before]`@BeforeEach` and [hotspot=After]`@AfterEach` annotations to perform any setup and teardown tasks for each
...
is controlled by marking them with the [hotspot=Order1 hotspot=Order2]`@Order` annotation. The value passed into the annotation
denotes the order in which the methods are run.

The test class contains three helper methods and two test methods:

The [hotspot=getResponse]`getResponse` helper method reuses the same line of code for retrieving a response from a
specific URL. This technique helps keep your code neat and organized.

The [hotspot=assertResponse]`assertResponse` helper method ensures that the response code that you receive is
valid (200).

The method [hotspot=visitLocalhost]`visitLocalhost` creates a [hotspot=targetResponse]`GET` request to the
`system` service, registering the `localhost` system.

The test [hotspot=testLinkForInventoryContents]`testLinkForInventoryContents` is responsible for asserting that
the correct HATEOAS link is created for the inventory contents.

Finally, the test [hotspot=testLinksForSystem]`testLinksForSystem` test is responsible for asserting that the correct
...
-------------------------------------------------------
 T E S T S
-------------------------------------------------------
Running it.guide-rest-hateoas.EndpointIT
Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 1.086 sec - in it.guide-rest-hateoas.EndpointIT
