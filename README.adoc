...
With the changes to the [hotspot file=0]`InventoryManager` and [hotspot file=1]`InventoryUtil` files, you will next modify the `...​/inventory/hosts/` URL in the [hotspot file=2]`InventoryResource` file.
...
At the URL http://localhost:9080/inventory/hosts[http://localhost:9080/inventory/hosts^] you will see links as built by the helper functions in the [hotspot file=1]`InventoryUtil` class.
A link points to an inventory entry of form `http://localhost:9080/inventory/hosts/<hostname>` with a [hotspot=self file=1]`self` relationship. 
A link also points to the `system` service with a [hotspot=properties file=1]`properties` relationship.
