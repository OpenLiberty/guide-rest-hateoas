...
[source, java, linenums, role='code_column hide_tags=comment']
----
include::finish/src/test/java/it/io/openliberty/guides/hateoas/EndpointTest.java[]
----
...
You can use the [hotspot=Before]`@Before` and [hotspot=After]`@After` annotations to perform any setup and teardown tasks for each
of your individual tests.

=== Writing the tests

Each test method must be marked with the [hotspot=Test]`@Test` annotation. Typically, the execution order of test
methods is not controlled, but if control is required, you can place methods in a single container test
method. This container method is then the only one marked with the [hotspot=Test]`@Test` annotation. The following
[hotspot=testSuite]`testSuite()` contains two test methods, which run in the order they appear:
...
The [hotspot=assertResponse]`assertResponse` method ensures that the response code that you receive is
valid (200).

The [hotspot=testLinksForSystem]`testLinksForSystem` test is responsible for asserting that the correct
...
Finally, the method [hotspot=visitLocalhost]`visitLocalhost` creates a [hotspot=targetResponse]`GET` request to the
