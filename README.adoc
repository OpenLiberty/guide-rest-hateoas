...
Begin by building your response JSON, which is composed of the `_links` array and the name of
the host machine.
...
The [hotspot=visitLocalhost]`visitLocalhost` method creates a [hotspot=targetResponse]`GET` request to the
`system` service, which registers the `localhost` system.
