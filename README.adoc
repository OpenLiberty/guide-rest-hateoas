...
----
#Add the `handler` method to the `InventoryResource` class.#
`src/main/java/io/openliberty/guides/microprofile/InventoryResource.java`
----
...
----
#Add the `UriInfo` object to the `InventoryResource` class.#
`src/main/java/io/openliberty/guides/microprofile/InventoryResource.java`
----
...
Let's implement the [hotspot=30-35 file=1]`buildHostJson` helper in the [hotspot=18-72 file=1]`InventoryUtil` class:

You're creating a JSON object that contains the name of your host system and the [hotspot=39 file=1]`_links` array, which
...
Create a test called [hotspot=48-68]`testLinkForInventoryContents`. This test is responsible for asserting that
the correct HATEOAS link is created for the inventory contents.

[role="code_command hotspot=48-68" subs="quotes"]
----
#Add the `testLinkForInventoryContents` method to the `EndpointTest` class.#
`src/test/java/it/io/openliberty/guides/hateoas/EndpointTest.java`
----

Write a [hotspot=110-112]`getResponse` helper method to reuse the same line of code for retrieving a response from a
specific URL. This technique helps keep your code neat and organized:

[role="code_command hotspot=110-112" subs="quotes"]
----
#Add the `getResponse` method to the `EndpointTest` class.#
`src/test/java/it/io/openliberty/guides/hateoas/EndpointTest.java`
----

Write another helper method called [hotspot=117-119]`assertResponse`. This method ensures that the response code that you receive is
valid (200):

[role="code_command hotspot=117-119" subs="quotes"]
----
#Add the `assertResponse` method to the `EndpointTest` class.#
`src/test/java/it/io/openliberty/guides/hateoas/EndpointTest.java`
----

Create a test called [hotspot=74-105]`testLinksForSystem`. This test is responsible for asserting that the correct
...
[role="code_command hotspot=74-105" subs="quotes"]
----
#Add the `testLinksForSystem` method to the `EndpointTest` class.#
`src/test/java/it/io/openliberty/guides/hateoas/EndpointTest.java`
----

Write a helper method called [hotspot=124-133]`visitLocalhost`. This method creates a [hotspot=132]`GET` request to the
