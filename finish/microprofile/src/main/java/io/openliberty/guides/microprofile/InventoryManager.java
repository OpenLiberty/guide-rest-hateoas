...
import io.openliberty.guides.util.ReadyJson;
import io.openliberty.guides.util.InventoryUtil;
...
public class InventoryManager { 
    ...
    // tag::getSystems[]
    public JsonArray getSystems(String invUri) {
        // inventory content
        JsonObject content = InventoryUtil.buildHostJson("*", invUri); 
        
        // collecting systems jsons
        JsonArrayBuilder jsonArray = inv.keySet().stream().map(host -> {
            return InventoryUtil.buildHostJson(host, invUri);
